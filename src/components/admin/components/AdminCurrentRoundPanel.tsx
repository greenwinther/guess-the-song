// src/components/admin/AdminCurrentRoundPanel.tsx

import styles from "@/components/admin/admin.module.css";
import type { AdminDashboardPayload } from "@/types/socket";

export default function AdminCurrentRoundPanel({
	dashboard,
					...
					<tbody>
						{dashboard.currentSongRows.map((row) => (
							<tr key={row.playerName} className="border-b border-border/60 last:border-b-0">
								...
								{dashboard.hasDetailLane && <td className="py-2 pr-3 text-text">{row.detailLabel}</td>}
								{dashboard.hasDetailLane && (
									<td className="py-2 pr-3 text-text/80">{row.detailLocked ? "Yes" : "No"}</td>
								)}
