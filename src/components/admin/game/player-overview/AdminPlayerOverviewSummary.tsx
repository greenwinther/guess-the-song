// src/components/admin/game/player-overview/AdminPlayerOverviewSummary.tsx

import type { AdminDashboardPayload } from "@/types/socket";

export default function AdminPlayerOverviewSummary({
	dashboard,
	reconnecting,
					...
					<div className="flex flex-wrap items-center justify-between gap-3">
						<h1 className="text-xl font-semibold text-text sm:text-[1.4rem]">
							{dashboard.activeSongIndex
								? `Song ${dashboard.activeSongIndex}`
								: "Waiting for song"}
						</h1>
						...
						<div className="min-w-0 space-y-1">
							...
							{dashboard.hasDetailLane && (
								<div className="text-sm">
									<span className="text-text/55">Bonus question:</span>{" "}
						...
						<div className="space-y-1 text-sm sm:text-right">
							...
							{dashboard.hasDetailLane && (
								<div>
									...
									<span className="text-text/82">{currentSongBonusAnswer ?? "-"}</span>
								</div>
							)}
						</div>
					</div>
